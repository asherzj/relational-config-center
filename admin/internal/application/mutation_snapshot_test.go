...
func TestTransactionalManagedTableMutationUsesOneCompleteSnapshotAndDatabaseAutoFill(t *testing.T) {
	...
	id, err := mutation.Add((AuthenticatedOperator{accountID: "00000000-0000-4000-8000-000000000001", roles: RoleAdmin}).Bind(t.Context()), "managed_items", domain.MutationContent{"name": jsonStringPointer("created")})
	if err != nil {
...
func TestTransactionalManagedTableMutationRejectsClientManagedFieldsInsteadOfOverwriting(t *testing.T) {
	session := validMutationSnapshotSession()
	mutation := NewManagedTableMutation(&memoryMutationSnapshotExecutor{session: session}, NewQueryPolicyTypeRegistry(), NewMutationPolicyTypeRegistry())
	_, err := mutation.Add((AuthenticatedOperator{accountID: "00000000-0000-4000-8000-000000000001", roles: RoleAdmin}).Bind(t.Context()), "managed_items", domain.MutationContent{
...
func TestTransactionalManagedTableMutationAuthorizationComesOnlyFromMutationPolicy(t *testing.T) {
	...
	mutation := NewManagedTableMutation(&memoryMutationSnapshotExecutor{session: session}, NewQueryPolicyTypeRegistry(), NewMutationPolicyTypeRegistry())
	_, err := mutation.Add((AuthenticatedOperator{accountID: "00000000-0000-4000-8000-000000000001", roles: RoleAdmin}).Bind(t.Context()), "managed_items", domain.MutationContent{"name": jsonStringPointer("denied")})
