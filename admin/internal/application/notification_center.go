	...
	err = r.store.ReadReleaseOrderList(ctx, func(reader ReleaseOrderListReader) error {
		...
		for {
			...
			for _, summary := range orders {
				order := domain.ReleaseOrder{ID: summary.ID, Version: summary.Version, State: summary.State, ApplicantID: summary.ApplicantID, TableNames: summary.TableNames, Approvals: summary.Approvals}
