...
func (r *ReleaseOrders) List(ctx context.Context, filter ReleaseFilter) ([]domain.ReleaseOrderSummary, error) {
	...
	for i := range orders {
		order := ReleaseOrder{ReleaseType: orders[i].ReleaseType, ID: orders[i].ID, Version: orders[i].Version, State: orders[i].State, ApplicantID: orders[i].ApplicantID, TableNames: orders[i].TableNames, Approvals: orders[i].Approvals}
		if err := r.reviewApprovals(ctx, &order); err != nil {
			return nil, err
		}
