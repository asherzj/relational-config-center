...
// TestReleaseTemplateBrowserSystemPath exercises the delivered Web page through
// a real Chromium session, same-origin Vite proxy, Admin process, and MySQL.
func TestReleaseTemplateBrowserSystemPath(t *testing.T) {
	...
	browser := exec.Command("node", filepath.Join(web, "e2e/release-templates.cjs"))
	browser.Dir = web
	...
	var creator, modifier, releaseType, monitors string
	if err := deliveryDB(t, driver).QueryRow(`SELECT creator,modifier,release_type,CAST(monitor_list AS CHAR) FROM rcc_release_templates WHERE code=?`, evidence.Code).Scan(&creator, &modifier, &releaseType, &monitors); err != nil {
		t.Fatal(err)
	}
	if creator != registered.Account.ID || modifier != registered.Account.ID || releaseType != "STANDARD" || monitors != "[]" {
		t.Fatalf("browser write audit mismatch: creator=%q modifier=%q type=%q monitors=%q", creator, modifier, releaseType, monitors)
	}
