...
func TestSchemaReadinessContinuouslyChecksStateAndCompleteStructureReadOnly(t *testing.T) {
	...
	for _, fault := range []struct{ name, apply, restore string }{
		...
		{"ahead", `UPDATE rcc_goose_db_version SET version_id=99 WHERE version_id=2`, `UPDATE rcc_goose_db_version SET version_id=2 WHERE version_id=99`},
		...
		{"release_digest", `UPDATE rcc_schema_migration_attempts SET release_digest=REPEAT('0',64) WHERE id=2`, ""},
		...
		t.Run(fault.name, func(t *testing.T) {
			...
			if fault.name == "release_digest" {
				if err := db.QueryRow(`SELECT release_digest FROM rcc_schema_migration_attempts WHERE id=2`).Scan(&digest); err != nil {
					t.Fatal(err)
				}
			}
			...
			if fault.restore != "" {
				...
				deliveryExec(t, db, `UPDATE rcc_schema_migration_attempts SET release_digest=? WHERE id=2`, digest)
			}
			assertReadiness(200)
		})
	}
	deliveryExec(t, db, `UPDATE rcc_schema_migration_attempts SET state='RUNNING',finished_at=NULL WHERE id=2`)
	assertReadiness(503)
...
func TestSchemaReadinessRejectsKnownOldRelease(t *testing.T) {
	...
	deliveryExec(t, db, `DELETE FROM rcc_goose_db_version WHERE version_id=2`)
	deliveryExec(t, db, `DELETE FROM rcc_schema_migration_attempts WHERE target_version=2`)
	status, _, body := p.request(t, "GET", "/health/ready", "", nil, "")
	if status != 503 {
