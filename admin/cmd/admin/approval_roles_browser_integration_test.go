...
func TestApprovalRoleBrowserSystemPath(t *testing.T) {
	_, driver := startCurrentIntegrationMySQL(t)
	owner := *driver
	owner.User = "root"
	// Test-only external database failure at the durable-result write boundary.
	deliveryExec(t, deliveryDB(t, &owner), `CREATE TRIGGER browser_reject_approval_request BEFORE INSERT ON rcc_approval_role_requests FOR EACH ROW BEGIN IF JSON_UNQUOTE(JSON_EXTRACT(NEW.result,'$.Description'))='数据库明确回滚保留输入' THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='browser request record failure'; END IF; END`)
	...
	browser := exec.Command("node", filepath.Join(web, "e2e/approval-roles.cjs"))
	browser.Dir = web
	...
	t.Logf("approval role browser: %s", result)
}
