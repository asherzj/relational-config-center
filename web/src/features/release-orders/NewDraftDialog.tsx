...
import {decodeReleaseRequest,defaultReleaseTitle,releaseRequests,releaseTitleError} from "../../api/release-orders";
...
export function NewDraftDialog({onClose}:{onClose:()=>void}) {
 ...
 const [table,setTable]=useState(initial&&"table_name" in initial.input?initial.input.table_name:""),[title,setTitle]=useState(initial&&"title" in initial.input?initial.input.title:"");
 const titleError=releaseTitleError(title);
 const navigate=useNavigate(),protection=useDraftProtection(Boolean(table||title),write.pending);
 return <Drawer open eyebrow="发布单" title={original?"保存发布草稿":"新建发布草稿"} onClose={()=>protection.requestLeave(onClose)} footer={<><Button disabled={write.pending} onClick={()=>protection.requestLeave(onClose)}>取消</Button><Button variant="primary" disabled={!allowed||(!original&&Boolean(titleError))||write.pending||write.blocked} onClick={async()=>{const saved=original?await write.retry():await write.send({...releaseRequests.create({title,table_name:table,items:[]}),label:"新建发布草稿"});if(saved)protection.afterSave(()=>{onClose();navigate(`/configuration/release-orders/${saved.id}`)})}}>{write.pending?"正在保存…":original?"确认并保存草稿":"创建空草稿"}</Button></>}>
 {original?<><p>原申请与请求标识已保留。保存会提交这份原申请，主单状态由正常读取更新。</p><PendingIntent item={original}/></>:<><p>先保存标题，再逐步添加明细。空草稿不占用目标；保存明细时开始占用。</p><div className="grid gap-4 mt-5"><label>起始表（可选）<NativeSelect aria-label="起始表（可选）" value={table} disabled={!allowed||write.pending||write.unresolved} onChange={event=>{setTable(event.target.value);setTitle(defaultReleaseTitle(event.target.value))}}><option value="">稍后从明细选择表</option>{tables.data?.filter(item=>item.enabled).map(item=><option key={item.tableName} value={item.tableName}>{item.tableName}</option>)}</NativeSelect></label><label htmlFor="new-draft-title">发布单标题<Input id="new-draft-title" value={title} required aria-invalid={Boolean(titleError)} aria-describedby={`new-draft-title-count${titleError?" new-draft-title-error":""}`} disabled={!allowed||write.pending||write.unresolved} onChange={event=>setTitle(event.target.value)}/></label><span id="new-draft-title-count" className="text-xs text-muted-foreground">{Array.from(title).length} / 100 字符</span>{titleError&&<small id="new-draft-title-error" className="field-error">{titleError}</small>}</div></>}{tables.isPending&&<LoadingState label="正在读取已启用的表…"/>}{tables.isError&&<ErrorState error={tables.error} onRetry={()=>void tables.refetch()}/>}{tables.isSuccess&&!tables.data.some(item=>item.enabled)&&<p>当前没有已启用的表，请联系管理员配置表规则。</p>} {Boolean(write.error)&&<ErrorState error={write.error}/>}</Drawer>;
}
