...
function RejectedRequest({item}:{item:PendingReleaseRequest}){
 ...
 return <><p role="alert">服务器已明确拒绝原请求。原申请保留，请查看最新状态与配置后决定是否重建。</p>
  ...
  {current&&<CurrentFieldDisplayProvider tableNames={releaseDetailTables(current)}><p>最新发布单版本：{current.version}，状态：{current.state}</p>{current.rollback_order_id&&<p>当前关联回滚发布单：<Link to={`/configuration/release-orders/${current.rollback_order_id}`}>{current.rollback_order_id}</Link></p>}<ReleaseDiff order={current}/></CurrentFieldDisplayProvider>}
