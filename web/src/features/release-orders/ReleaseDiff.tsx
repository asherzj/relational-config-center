...
export function ReleaseDiff({order,beforeLabel="服务器原值",proposedLabel="申请值"}:{order:Pick<ReleaseOrder,"items">;beforeLabel?:string;proposedLabel?:string}){
 ...
 return <section aria-label="变更内容" className="min-w-0">
  ...
  {order.items.slice(page*releasePageSize,(page+1)*releasePageSize).map((item,offset)=>{
   ...
   const fields=item.fields.filter(field=>!onlyChanges||item.operation!=="MODIFY"||!(field.proposed_state==="omitted"||field.before_state===field.proposed_state&&(field.before_state==="sql_null"||field.before_state==="value"&&field.before===field.proposed)));
   return <section key={index} aria-label={`明细 ${index+1}`}><details open={expanded.has(index)} className="release-diff-item mb-3 min-w-0 rounded-lg border">
    ...
    <div className="min-w-0 px-4 pb-4"><p className="mb-3 text-xs text-muted-foreground break-all">记录基线 {item.expected_record_version||"尚无已知 id"}</p>
     {fields.length===0?<p>没有确定发生变化的字段；取消“仅看变更”可查看完整申请。</p>:<div className="table-scroll"><Table className="min-w-[560px] table-fixed" containerProps={{tabIndex:0,role:"region","aria-label":`明细 ${index+1} 字段对比，可横向滚动`,className:"release-diff-scroll"}}><TableHeader><TableRow><TableHead className="w-1/4">字段</TableHead><TableHead>{beforeLabel}</TableHead><TableHead>{proposedLabel}</TableHead></TableRow></TableHeader><TableBody>{fields.map(field=><TableRow key={field.name}><TableHead scope="row" className="whitespace-normal break-all font-mono">{field.name}<small className="block text-muted-foreground font-normal">{field.type}</small></TableHead><TableCell className="release-diff-before align-top font-mono"><ReleaseValue state={field.before_state} value={field.before} type={field.type}/></TableCell><TableCell className="release-diff-after align-top font-mono"><ReleaseValue state={field.proposed_state} value={field.proposed} type={field.type}/></TableCell></TableRow>)}</TableBody></Table></div>}
    </div>
   </details></section>;
  })}
 </section>;
