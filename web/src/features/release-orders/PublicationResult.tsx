...
function PersistedFieldValue({field,name}:{field:Field|undefined;name:string}){
 const currentDisplay=useCurrentFieldDisplayContext();
 if(!field)return <span className="text-muted-foreground">不存在</span>;
 return <CurrentFieldValue display={currentDisplay.display} name={name} value={field.value}>{fieldValue(field)}</CurrentFieldValue>;
}
...
export function PublicationResult({result,people={}}:{result:Result;people?:Record<string,string>}) {
  ...
  return (
    <section className="my-6 break-all" aria-label="发布结果">
      ...
      {result.commands.slice(page*releasePageSize,(page+1)*releasePageSize).map(command => (
        <article key={command.sequence} className="my-4">
          <h3>{command.operation} · 记录 {command.id === "" ? '""（空字符串）' : command.id}</h3>
          <p>变更顺序：{command.sequence} · 记录并发版本：{command.record_version}</p>
          ...
          <Table className="min-w-[560px] table-fixed" containerProps={{tabIndex:0,role:"region","aria-label":`变更顺序 ${command.sequence} 实际结果，可横向滚动`,className:"release-diff-scroll"}}>
            ...
            <TableBody>{orderDisplayedFields(currentDisplay.display,[...new Set([...command.before.fields.map(field=>field.name),...command.final.fields.map(field=>field.name)])],name=>name).map(name=>{
              const before=command.before.fields.find(field=>field.name===name),final=command.final.fields.find(field=>field.name===name);
              return <TableRow key={name}><TableHead scope="row" className="whitespace-normal break-all"><CurrentFieldName display={currentDisplay.display} name={name} detail={(before??final)?.type}/></TableHead><TableCell className="align-top whitespace-pre-wrap font-mono"><PersistedFieldValue name={name} field={before}/></TableCell><TableCell className="align-top whitespace-pre-wrap font-mono"><PersistedFieldValue name={name} field={final}/></TableCell></TableRow>;
            })}</TableBody>
          </Table>
        </article>
      ))}
