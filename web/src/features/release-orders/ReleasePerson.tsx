export function ReleasePerson({id,name}:{id:string;name?:string}){
 ...
 return <span className="inline-flex min-w-0 items-center gap-2 align-middle">
  ...
  <span className="min-w-0"><strong className="block font-medium text-foreground">{label}</strong><code className="block break-all text-xs text-muted-foreground">{id}</code></span>
 </span>;
}
