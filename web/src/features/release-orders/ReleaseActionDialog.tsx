...
export function ReprepareDraftDialog({order,onClose}:{order:ReleaseOrder;onClose:()=>void}){
 ...
 </Drawer><ConfirmDialog open={confirming} title="取消旧单并创建新草稿？" description="确认后，已批准的旧发布单会被取消并释放全部目标；系统会创建由你申请的新草稿，旧审批不会沿用。" confirmLabel={"取消旧单并创建新草稿"} destructive pending={write.pending} confirmDisabled={!allowed||!snapshot&&!write.unresolved} onCancel={()=>setConfirming(false)} onConfirm={()=>void apply()}>{Boolean(write.error)&&<ErrorState error={write.error}/>} {write.unresolved&&<p role="alert">原正文和请求标识已保留；再次点击原操作将提交同一份申请。</p>}</ConfirmDialog></>;
}
