...
function ReleaseDetail({id}:{id:string}){
 ...
 <ReleaseDiff order={order}/>{order.publication&&<PublicationResult result={order.publication}/>}<h2 className="text-lg font-semibold mb-3">操作历史</h2><ol className="grid gap-3">{order.history.map(event=><li key={event.version} className="border-b pb-3 break-all">{event.action} · 版本 {event.version}<p>账号：{event.actor_id}</p><time>{event.at}</time>{event.reason&&<p>{event.reason}</p>}</li>)}</ol>
