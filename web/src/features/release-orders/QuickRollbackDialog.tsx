...
export function QuickRollbackDialog({order,onClose}:{order:ReleaseOrder;onClose:()=>void}){
 const [version]=useState(order.version),[reason,setReason]=useState("");
 ...
 const allowed=useAccountRole("PUBLISHER")&&order.allowed_actions.includes("quick-rollback");
 const titleID=useId(),descriptionID=useId(),reasonID=useId(),errorID=useId();
 const cancelRef=useRef<HTMLButtonElement>(null);
 const preview=useQuery({queryKey:["quick-rollback-preview",order.id,version],queryFn:()=>releaseOrders.quickRollbackPreview(order.id,version),enabled:!write.unresolved&&!rejected,retry:false,refetchOnWindowFocus:false,refetchOnMount:"always"});
 ...
 return <ModalSurface open onClose={close} pending={write.pending} labelledBy={titleID} describedBy={descriptionID} dismissLabel="取消快速回滚确认" initialFocusRef={cancelRef} className="grid w-full min-w-0 grid-rows-[auto_minmax(0,1fr)_auto] gap-5 overflow-hidden p-6 sm:max-w-[980px]">
  ...
  <DialogDescription id={descriptionID}>核对当前配置将恢复成的内容，再填写原因并确认。数据库外部改动、规则或表结构变化仍可能阻止恢复。</DialogDescription></header>
  ...
  {preview.isFetching?<LoadingState label="正在读取整单恢复预览…"/>:preview.isError?<ErrorState error={preview.error} onRetry={()=>void preview.refetch()}/>:preview.data&&<>
   <p>本次恢复涉及全部 {preview.data.items.length.toLocaleString("en-US")} 项，无需再次审批。成功后原单和回滚结果均结束，释放目标记录的占用。</p>
