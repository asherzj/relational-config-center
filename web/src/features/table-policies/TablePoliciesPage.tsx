  ...
  return (
    <main className="workspace table-policies-workspace">
      <div className="page-heading">
        <div><h1>表策略分配</h1><p>从真实 Managed Data Source 发现表，并分配可复用的 Active Query Policy 与 Mutation Policy。</p></div>
      ...
      <section className="discovery-panel" aria-label="Database Table Discovery">
        ...
        {discovery.isPending ? <LoadingState label="正在读取真实数据库表…" /> : discovery.isError ? <ErrorState error={discovery.error} onRetry={() => void discovery.refetch()} /> : !discovery.data.length ? (
          <div className="feedback-state"><strong>没有可发现的数据库表</strong><span>Managed Data Source 当前没有可用于策略分配的真实表。</span></div>
        ) : (
      ...
      <section className="catalog" aria-label="Table Policy 目录">
        <div className="catalog-toolbar">
          <label className="search-field"><Search size={16} /><span className="sr-only">筛选表策略</span><input type="search" aria-label="筛选表策略" placeholder="筛选表名、Policy Code 或修改人" value={filter} onChange={(event) => setFilter(event.target.value)} /></label>
        </div>
        {policies.isPending ? <LoadingState label="正在读取 Table Policy 目录…" /> : policies.isError ? <ErrorState error={policies.error} onRetry={() => void policies.refetch()} /> : !policies.data.length ? <EmptyState entity="表策略" /> : !filteredPolicies.length ? <div className="feedback-state"><strong>没有匹配的表策略</strong><span>尝试调整筛选条件。</span></div> : (
          <div className="table-scroll"><table className="policy-table table-policy-table"><thead><tr><th>表名</th><th>Query Policy Code</th><th>Mutation Policy Code</th><th>状态</th><th>创建人</th><th>修改人</th><th>修改时间</th><th>操作</th></tr></thead><tbody>
        ...
        <footer className="catalog-footer"><span>共 {policies.data?.length ?? 0} 个 Table Policy</span><span>完整目录 · 客户端筛选</span><Button className="catalog-refresh" variant="ghost" icon={<RefreshCw size={15} />} onClick={() => { void policies.refetch(); void discovery.refetch(); }}>刷新</Button></footer>
      </section>
