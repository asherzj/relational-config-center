...
export function MutationPolicyDrawer({ code, commandsBlocked, onRequestCommand }: Props) {
  ...
  const verifyCurrentState = async () => {
    try {
      await queryClient.refetchQueries({ queryKey: mutationPolicyKeys.all }, { throwOnError: true });
    } catch {
  ...
  const submit = (value: MutationPolicyDraft | MutationPolicyMetadata) => {
    if (uncertain || commandsBlocked) return;
