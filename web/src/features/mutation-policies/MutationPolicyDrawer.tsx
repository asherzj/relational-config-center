...
export function MutationPolicyDrawer({ code, onRequestCommand }: Props) {
  ...
  const title = useMemo(() => {
    if (mode === "create") return "新建变更策略草稿";
    if (mode === "replace") return "编辑变更策略草稿";
    if (mode === "metadata") return "更新变更策略信息";
    return "变更策略详情";
  }, [mode]);

  const submit = (value: MutationPolicyDraft | MutationPolicyMetadata) => {
    if (mode === "create") create.mutate(value as MutationPolicyDraft, { onSuccess(created) { showToast("变更策略草稿已创建"); navigate(`/platform/mutation-policies/${encodeURIComponent(created.code)}`); } });
    else if (mode === "replace" && code) replace.mutate({ code, draft: value as MutationPolicyDraft }, { onSuccess() { showToast("变更策略草稿已更新"); navigate(`/platform/mutation-policies/${encodeURIComponent(code)}`); } });
    else if (mode === "metadata" && code) metadata.mutate({ code, metadata: value as MutationPolicyMetadata }, { onSuccess() { showToast("变更策略显示信息已更新"); navigate(`/platform/mutation-policies/${encodeURIComponent(code)}`); } });
  };

  let content: ReactNode;
  if (!creating && detail.isPending) content = <LoadingState label="正在读取变更策略…" />;
  else if (!creating && detail.isError) content = <ErrorState error={detail.error} onRetry={() => void detail.refetch()} />;
  else content = <>{!supported && <div className="inline-alert"><AlertCircle size={18} /><strong>Type Registry 未确认 {policy?.typeCode} 的完整 ADD/MODIFY/DELETE 能力；执行规则失败关闭，仅可安全查看或更新元数据。</strong></div>}<MutationPolicyForm mode={mode} policy={policy} typeCodes={(types.data ?? []).map((type) => type.code)} serverError={serverError} onSubmit={submit} /></>;

  let footer: ReactNode = <Button onClick={close}>关闭</Button>;
  if (mode === "create" || mode === "replace" || mode === "metadata") footer = <><Button variant="primary" type="submit" form="mutation-policy-form" disabled={pending || (mode === "create" && !types.data?.some((type) => supportedMutationPolicyTypes.has(type.code)))}>{pending ? "正在保存…" : mode === "create" ? "创建草稿" : "保存"}</Button><Button onClick={close} disabled={pending}>取消</Button></>;
  ...
  return <Drawer open={Boolean(code)} title={title} eyebrow="变更策略" onClose={close} footer={footer}>{content}</Drawer>;
}
