...
export function MutationSuccessDialog({ outcome, retryPending, onRetry, onClose }: { outcome: ManagedDataMutationOutcome | null; retryPending: boolean; onRetry: () => void; onClose: () => void }) {
  ...
  useModalFocus({ open: Boolean(operation), dialogRef, onEscape: retryPending ? undefined : onClose });
  ...
  return (
    <div className="modal-layer change-set-layer">
      ...
      <section ref={dialogRef} tabIndex={-1} className="mutation-success-dialog" role="dialog" aria-modal="true" aria-label={`${outcome.operation} 写入结果`} data-modal-surface="true">
