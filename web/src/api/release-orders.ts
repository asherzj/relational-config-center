...
export type ReleaseStateAction="submit"|"approve"|"reject"|"cancel";
export const releaseActionLabels={submit:"提交审批",approve:"批准发布单",reject:"拒绝发布单",cancel:"取消发布单",copy:"复制新草稿"};
export const releaseActionRole=(action:string)=>action==="approve"||action==="reject"?"APPROVER" as const:"EDITOR" as const;

export const releaseRequests={
 action:(action:ReleaseStateAction,id:string,expectedVersion:string,reason=""):ReleaseRequestEnvelope=>({path:`/api/v1/release-orders/${encodeURIComponent(id)}/${action}`,method:"POST",body:JSON.stringify({expected_version:expectedVersion,...(action!=="submit"?{reason}:{})})}),
