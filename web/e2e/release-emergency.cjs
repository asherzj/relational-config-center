...
(async()=>{
 ...
 try{
  ...
  await page.getByText('应急发布',{exact:true}).first().waitFor();
  ...
  await page.getByText('应急发布',{exact:true}).first().waitFor();order=await api(context,'GET',path);
  assert.equal(order.release_type,'EMERGENCY');assert.equal(order.table_flows.length,2);
  ...
  await page.getByRole('heading',{name:title,exact:true}).waitFor();await page.getByText('应急原因',{exact:true}).first().waitFor();
  await page.setViewportSize({width:390,height:844});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);
  assert.equal(await page.getByText('逐表审批进度',{exact:true}).count(),0);assert.equal(await page.getByText('最近审批人',{exact:true}).count(),0);assert.equal(await page.getByText('尚无批准记录',{exact:true}).count(),0);assert.equal(await button(page,'批准发布单').count(),0);
