...
(async()=>{
 ...
 try{
  ...
  const queries=[],requestIDs=new Map(),selectedQueries={};
  page.on('request',request=>{if(new URL(request.url()).pathname===queryPath&&request.method()==='POST'){requestIDs.set(request,queries.length);queries.push({sequence:queries.length,requested_at:Date.now(),body:request.postDataJSON()});}});
  page.on('response',response=>{const id=requestIDs.get(response.request());if(id!==undefined)Object.assign(queries[id],{status:response.status(),responded_at:Date.now()});});
  ...
  const submit=async()=>{const pending=page.waitForResponse(r=>new URL(r.url()).pathname===queryPath&&r.request().method()==='POST');await filters.getByRole('button',{name:'查询',exact:true}).click();const response=await pending;assert.equal(response.status(),200);return {sequence:requestIDs.get(response.request()),request:response.request().postDataJSON(),data:await response.json()};};
  ...
  selectedQueries.first=first.sequence;
  ...
  const clearing=page.waitForResponse(response=>new URL(response.url()).pathname===queryPath&&response.request().method()==='POST');
  await filters.getByRole('button',{name:'清空',exact:true}).click();
  const cleared=await clearing;assert.equal(cleared.status(),200);assert.deepEqual(cleared.request().postDataJSON().conditions,[]);
  selectedQueries.clear=requestIDs.get(cleared.request());
  ...
  selectedQueries.second=second.sequence;fs.writeFileSync(path.join(output,'query-requests.json'),JSON.stringify({queries,selectedQueries},null,2));
