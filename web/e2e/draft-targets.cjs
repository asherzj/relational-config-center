...
const base=process.env.RCC_WEB_URL,table='draft_browser_items',longField='long_'+'x'.repeat(59),output=process.env.RCC_E2E_OUTPUT;
(async()=>{
 ...
 try{
  ...
  await api(admin,'POST','/api/v1/mutation-policies',{code:'draft_browser_mutation_v1',name:'草稿管控验收',description:'',type_code:'single_table_mutation',allow_add:true,allow_modify:true,allow_delete:true,create_time_field:'stamp'},201);
  await api(admin,'POST','/api/v1/mutation-policies/draft_browser_mutation_v1/activate',{});
  await api(admin,'POST','/api/v1/table-policies',{table_name:table,query_policy_code:'notification_page_query_v1',mutation_policy_code:'draft_browser_mutation_v1'},201);
  ...
  await page.goto(`${base}/configuration/release-orders`);let tableReads=0;await page.route('**/api/v1/table-policies',async route=>{if(++tableReads===1)await route.abort();else await route.continue()});await button(page,'新建草稿').click();await page.getByLabel('发布单标题',{exact:true}).fill('读取失败时保留标题');await button(page,'重试').click();assert.equal(await page.getByLabel('发布单标题',{exact:true}).inputValue(),'读取失败时保留标题');await page.getByLabel('草稿表',{exact:true}).selectOption(table);await page.unroute('**/api/v1/table-policies');await page.getByLabel('发布单标题',{exact:true}).fill('   ');assert.equal(await page.getByLabel('发布单标题',{exact:true}).getAttribute('aria-invalid'),'true');assert.equal(await button(page,'创建空草稿').isDisabled(),true);await page.getByLabel('发布单标题',{exact:true}).fill('T2 分页草稿');await button(page,'创建空草稿').click();await page.getByRole('heading',{name:'T2 分页草稿',exact:true}).waitFor();
  const id=new URL(page.url()).pathname.split('/').at(-1);let draft=await api(editor,'GET',`/api/v1/release-orders/${id}`);assert.deepEqual(draft.items,[]);assert.equal(await button(page,'提交审批').isDisabled(),true);
  draft=await api(editor,'PUT',`/api/v1/release-orders/${id}`,{title:draft.title,table_name:table,expected_version:draft.version,changes:{upserts:Array.from({length:25},(_,index)=>({operation:'MODIFY',id:String(index+1),expected_record_version:'0',content:{label:`proposal-${index+1}`}}))}});
  check('编辑者创建空草稿，增量添加25项后使用同一整单版本');
  const blocker=await api(admin,'POST','/api/v1/release-orders',{title:'T2 冲突负责人',table_name:table,items:[{operation:'ADD',content:{id:'500',code:'occupied',label:'owner'}}]},201);
  ...
  await page.getByLabel('code 申请值',{exact:true}).fill('available');await button(page,'查看最新发布单').click();await button(page,'基于最新发布单重建').click();await button(page,'保存草稿修改').click();await page.getByRole('heading',{name:'T2 分页草稿',exact:true}).waitFor();await page.getByRole('heading',{name:`编辑 ${table} 草稿`,exact:true}).waitFor({state:'hidden'});
  draft=await api(editor,'GET',`/api/v1/release-orders/${id}`);assert.equal(draft.items[20].content.code,'available');assert.equal(draft.items[0].content.label,'proposal-1');
  ...
  await button(page,'编辑草稿').click();await page.getByLabel('label 申请值',{exact:true}).fill('retained detail one');await button(other,'保存草稿修改').click();await other.getByRole('heading',{name:`编辑 ${table} 草稿`,exact:true}).waitFor({state:'hidden'});
  ...
  await button(page,'查看最新发布单').click();await button(page,'基于最新发布单重建').click();await button(page,'保存草稿修改').click();await page.getByRole('heading',{name:`编辑 ${table} 草稿`,exact:true}).waitFor({state:'hidden'});
  draft=await api(editor,'GET',`/api/v1/release-orders/${id}`);assert.equal(draft.items[0].content.label,'retained detail one');assert.equal(draft.items[1].content.label,'other window detail two');
  await button(page,'编辑草稿').click();await page.getByLabel('定位编辑明细',{exact:true}).fill('21');await button(page,'上移明细').focus();await page.keyboard.press('Enter');await button(page,'保存草稿修改').click();await page.getByRole('heading',{name:`编辑 ${table} 草稿`,exact:true}).waitFor({state:'hidden'});
  const reordered=await api(editor,'GET',`/api/v1/release-orders/${id}`);assert.equal(reordered.items[19].detail_id,draft.items[20].detail_id);
