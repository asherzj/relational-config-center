...
(async()=>{
 ...
 try{
  ...
  // Returning from detail remounts the default paginated list; query on every return.
  const findCancelledDraft=async()=>{await page.getByLabel('表名',{exact:true}).fill('stage1_acceptance_items');await page.getByLabel('申请人账号 ID',{exact:true}).fill(identity.account.id);await page.getByLabel('状态',{exact:true}).selectOption('CANCELLED');await page.getByRole('button',{name:'查询发布单',exact:true}).click();await page.getByRole('link',{name:committed.id,exact:true}).waitFor();};
