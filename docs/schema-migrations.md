...
当前基线包括历史 013 审计列、014 字段规则及 015～017 的原单明细、执行、管控键、表引用和主单字段收敛。已发布 Goose 00001～00003 的 SQL 和清单保持不变；追加 00004/00005 分阶段完成整张表的可恢复变更。审批角色管理追加 `00006_approval_roles.sql` 和完整清单，建立角色、成员、幂等结果及永久引用结构。接管边界固定在已发布版本 5，不随后续功能迁移移动。表审批分配由 00007 增量提供；个人审批通知由 00008 增量及完整清单提供。新库顺序执行八个版本；已有 Goose 版本通过 `up` 升级。未接管库必须先达到当前完整结构，再显式 `baseline` 登记 1～5；本构建返回 `pending`，随后显式 `up` 执行 00006～00008。旧发布单业务 JSON 保留原样且不兼容读取；账号、会话、规则、版本及技术通知保留，详见[发布升级](admin-release-upgrade.md)。
