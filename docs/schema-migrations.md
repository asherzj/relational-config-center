...
历史接管基线包括历史 013 审计列、014 字段规则及 015～017 的原单明细、执行、管控键、表引用和主单字段收敛，对应已经发布的 Goose 00001～00005。未接管库必须先达到这份完整历史结构，再显式 `baseline` 登记 1～5；接管边界不会随本构建新增迁移向后移动。之后显式运行 `up` 执行本构建追加的版本。新库顺序执行本构建嵌入的全部迁移；已有 Goose 版本通过 `up` 升级。旧发布单业务 JSON 保留原样且不兼容读取；账号、会话、规则、版本及技术通知保留，详见[发布升级](admin-release-upgrade.md)。
