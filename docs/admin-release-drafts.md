...
并发相同请求由数据库唯一键收敛；新状态动作锁定单据并校验版本。草稿、历史与幂等结果同一事务提交，失败全部回滚，不保留半张草稿。发布/快速回滚的已确认失败另行绑定原请求摘要并追加失败历史，result 保持 NULL，详见[执行失败契约](admin-release-approvals.md#执行失败与失败历史86)。草稿内容、完整目标集合及未结束表引用一起提交；占用失败保留原草稿、原占用与版本。草稿保存没有业务行写入方法。

Web 在请求发送前将原键、路径和完整申请内容在一个 IndexedDB 事务中持久保存，以 Account ID 隔离；事务完成前不发送请求，Cookie/CSRF 不进入该记录。同账号窗口同步存储状态，不自动重推或覆盖未确认原请求。刷新或重新登录后从原草稿/发布操作查看原申请，再次点击相同业务按钮安全重推；没有独立结果确认入口。未知结果后的 401/403 或一次详情未找到均不能删除原请求或自动换键；切换账号不能重放上一账号请求。结果确定成功后仅按原账号和原键删除临时请求，未发出的普通编辑仍沿用现有未保存保护。日志不可读时发布功能提示错误并阻止相关写入，账号与无关读取仍可使用。

原请求写重试经过服务器去重后，若明确返回 `release_version_conflict`、`record_version_conflict` 或 `release_state_invalid`，说明该标识没有已成功业务结果；页面解除未知状态，但将原申请持久保留为待重建内容；刷新后的冲突审阅仍可查看完整原意，必须先读取最新单据/记录并明确确认重建，才能用新标识保存。鉴权失败或一次详情读取失败不能据此丢弃原标识。
