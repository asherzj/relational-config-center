...
try {
  ...
  for (let iteration=1; iteration<=10; iteration++) {
    ...
    await originalReleaseActions.repeatReleaseAction(page,'执行发布','确认发布到数据库');assert.equal((await repeated).status(),200);await page.getByRole('button',{name:'确认发布到数据库',exact:true}).waitFor({state:'hidden'});await releaseState(page,'已发布待完结');
