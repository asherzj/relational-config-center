...
固定功能基点 `ab7414ea96ffd89cb7ba1ac67e35a4297f1ba087`，在独立 `codex/issue-55-upgrade-delivery` 分支集成当时最新主分支 `5df095c57769aed828cba9f617adcbfad48d7861`。18 处真实冲突逐处保留正式发布语义和主分支的输入、真实 ID、恢复及浏览器改进；没有恢复旧记录直写实现。用户随后于 2026-09-08 10:23:04 UTC 合入 [PR #56](https://github.com/asherzj/relational-config-center/pull/56)；隔离修订分支已接收最新 main `e43fe2cdbe616ab5f612c715aff8ec536ee21a69`，连同 PR #57 的领域说明、路线图和独立静态预览完整保留。父规格 #48 保持开放，未执行部署。

本记录按持久原始输出填写。主体功能已合入 main；后续浏览器分页定位修订完成本地验收，新的最终提交 CI 仍须实际成功。最终状态见 #55 完成记录；远端最终提交 CI 的实际结论固定记录在 [#55 完成评论](https://github.com/asherzj/relational-config-center/issues/55)，不通过 CI 后再追加文档提交改变已验证候选。全部 48 项的主要交付归属及前七单固定提交证据见[验收索引](2026-09-08-release-acceptance-index.md)。
...
`make test-browser-acceptance` 在最终相同字节的候选上以 `RCC_E2E_SUITE=all`、`RCC_E2E_ENGINES=chromium,firefox,webkit` 实际执行，2026-09-08 10:54:16～11:01:45 UTC 完成，命令耗时 449.721 秒、退出 0。数据库完整夹具逐字节一致，所有临时数据、监听器、容器和数据卷完成清理，生成凭据检查通过。
...
首轮远端 CI 的 Web、Go 和 MySQL 均通过，浏览器在 WebKit 角色页等待新成员时失败：页面正常返回 25 个账号，目标不在第一页。后续完整复验又发现草稿详情返回列表会重置筛选，同样不能假定单号位于默认 20 项第一页。三个验收脚本现通过实际可见账号检索、表名/申请人/状态筛选或单号查询定位，等待准确 GET 200 并核对永久账号 ID 或唯一单号，再继续原 UI 角色分配、取消历史、390px 展示和原键恢复路径。草稿两次返回和三引擎账号定位均有安全响应证据，没有延长等待、随机重试或绕过权限。上述最新完整浏览器结果包含这些修订；保留的 Go 浏览器入口也已重新执行通过。
...
| 正式完整浏览器入口 | 退出 0，449.721s，三引擎全套且清理通过 |
| 保留的 `make test-browser` 系统入口 | 退出 0；命令 107.159s / Admin 105.495s，真实账号日志及共享回滚夹具通过 |
...
后续提交 `8797a00` 的 [CI run 34219870716](https://github.com/asherzj/relational-config-center/actions/runs/34219870716) 中 Web、Go 和三引擎浏览器通过，MySQL 在 40 分钟总时限处失败。当时 `TestReleaseDraftMissingIdentityAndTombstone` 只运行了 1 秒，正在等待新建 MySQL 就绪；本轮日志记录 165 次容器启动、164 次就绪，就绪耗时中位数 12 秒。本地完整运行的 219 次启动全部就绪，中位数 7 秒；首轮远端相同 Go 实现曾以 2117.342 秒完成 Admin 包。当前将整组测试预算改为 60 分钟、MySQL CI 任务改为 65 分钟，覆盖累计隔离启动成本的波动。Admin 集成夹具的 2 分钟期限、生产默认 4 秒期限、完整测试集合和全部断言保持不变；新的提交仍须通过实际 CI，此次失败不计为通过。
...
独立 Spec 评审未发现具体问题；Standards 未发现硬性违规，有一项低优先主观建议：共享 WriteRecovery 组件集中目录字段展示。当前展示分支不恢复旧写入口或隐式重试，既有组件和真实恢复验收已通过，本期接受该非阻断建议并保持执行代码不变。两轴报告形成时完整 MySQL 尚在运行；最终证据签核及远端 CI 结论以 #55 后续记录为准。后续三个脚本修订另经独立 Standards / Spec 评审，各无发现；实际新完整浏览器运行前后 449 个文件完全一致，当轮执行类别 325 个（包含接收的独立静态预览）。此后仅调整整套集成测试及 CI 的总时间预算，并更新对应说明；生产 Go/Web 源码、测试源码和构建目标保持原验证字节。三个改动的浏览器脚本已通过新完整入口；保留入口也已重新通过。原失败日志与初审时的进行中事实完整保留。
...
Environment、跨表发布、规则目录审批、业务 Schema 编辑、灰度、Worker、Server/Client 实际分发、版本大盘和历史自动清理仍不在本期。Notion 项目总表按最终 CI 及实际 Admin 已交付范围更新，功能完成状态与远端验证链接查 [#55 完成记录](https://github.com/asherzj/relational-config-center/issues/55)。本修订提交时新的远端 CI 及 Notion 完成更新尚待执行；Agent 审计与 Runtime 工作保持未来范围。
