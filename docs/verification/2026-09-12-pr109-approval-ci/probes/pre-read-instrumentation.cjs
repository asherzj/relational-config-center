...
(async () => {
  ...
  try {
    ...
    const fixtureRole = await createFixtureApprovalRole(context, base, `Accessibility review ${randomUUID()}`, [approverAccount.accountID], [table]);
    ...
    const diagnosticReviewer = await (await approvalContext.request.get(`${base}/api/v1/auth/session`)).json();
    const diagnosticAssignment = await (await authenticatedRequest(context, base, `/api/v1/table-policies/${table}/approval-roles`)).json();
    const diagnosticOrder = await (await authenticatedRequest(approvalContext, base, `/api/v1/release-orders/${invalidOrder.id}`)).json();
    const diagnosticRoles = [];
    for (const id of new Set([fixtureRole.id, ...(diagnosticOrder.approvals ?? []).flatMap(approval => approval.roles.map(role => role.id))])) {
      diagnosticRoles.push(await (await authenticatedRequest(context, base, `/api/v1/approval-roles/${id}`)).json());
    }
    await fs.writeFile(`${output}/approval-diagnostic.json`, JSON.stringify({ engine: engineName, expectedActorID: account.accountID, expectedReviewerID: approverAccount.accountID, createdOrder: await created.json(), reviewer: diagnosticReviewer.account, fixtureRole, currentAssignment: diagnosticAssignment, submitted: invalidOrder, reviewerOrder: diagnosticOrder, liveRoles: diagnosticRoles }, null, 2));
    console.log('[DEBUG-approval] saved actor, assignment, role and order evidence');
