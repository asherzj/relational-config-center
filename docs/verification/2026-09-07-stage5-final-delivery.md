...
> 历史状态更正：本报告记录的旧工作包曾把 Issue #22 写成 OPEN，并把 MR #42 写成待评审。实际 GitHub 状态为 MR #42 已于 2026-09-07 04:04:21Z 合并，Issue #22 已于 04:04:22Z 关闭；下文涉及旧分支的描述保留为当时的验收快照。
...
本阶段读取 #22 时，Issue 为 `OPEN`，标签为 `ready-for-agent`，没有评论。当时继续保持 OPEN 的理由是：完整 Web 基线已在 `main`，但本工作包中的非法 ENUM 稳定错误修复、焦点与未保存保护、规则解释和本阶段列表文案修复尚未全部合入 `main`。该后续状态现已落实：MR #42 已合并，#22 已关闭；原阶段没有评论、改标签、关闭 Issue 或创建 PR。
