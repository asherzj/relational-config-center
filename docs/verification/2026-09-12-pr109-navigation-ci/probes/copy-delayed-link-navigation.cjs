...
(async()=>{
 ...
 try{
  ...
  for(let iteration=0;iteration<3;iteration++){
   ...
   await page.getByLabel('基本信息',{exact:true}).getByRole('link',{name:source.id,exact:true}).click();
   await page.waitForURL(`**/configuration/release-orders/${source.id}`);const forward=page.getByRole('link',{name:copied.id,exact:true});await forward.waitFor();await forward.click();
