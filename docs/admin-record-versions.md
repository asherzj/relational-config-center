...
首次升级必须停掉全部旧 Admin 及外部业务写入，备份业务表和控制表，执行 `deploy/mysql/migrations/009-record-versions.sql`，按[完整升级指南](admin-release-upgrade.md)补齐其他适用历史控制步骤和 013，显式 `schema-migrate baseline` 并确认 `status` 为 `current` 后，部署同时要求期望版本的 Admin/Web，再恢复服务。迁移可重跑，既有业务数据与控制版本不变；旧客户端缺版本会被拒绝。旧二进制不能与新版本同时写入，也不能回退旧二进制后继续写。
