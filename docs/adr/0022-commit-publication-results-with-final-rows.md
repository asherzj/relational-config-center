...
已批准发布在一个 MySQL 事务中保存配置、记录版本、发布前值与实际最终行、Publication Command、Table Version、发布历史、目标占用变化、通知和幂等结果。默认值、生成列和允许的目标行触发器必须执行后读取，不能用申请内容推算；以表内进度行串行分配版本和游标，避免全局自增分配顺序与提交顺序相反。COMMIT 无法确认时保留原请求标识，由数据库持久结果收敛。
