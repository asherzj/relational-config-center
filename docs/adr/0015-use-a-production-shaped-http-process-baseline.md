...
> 本地账号交付由 [ADR-0017](./0017-introduce-local-accounts-in-the-trusted-network.md) 取代下述部署 Token、免认证和 CORS 约定；正常业务入口现在使用同源 Cookie 会话与 CSRF。Readiness 按 [ADR-0024](0024-adopt-goose-at-a-verified-control-schema-baseline.md) 只读检查迁移版本、确认状态及完整必要控制结构，结构不完整拒绝启动并给出迁移指引；账号目录为空仍正常就绪。以下其余进程、超时、日志和优雅停机约定继续有效。
